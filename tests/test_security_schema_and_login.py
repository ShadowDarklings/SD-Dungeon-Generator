...
def test_post_api_runs_persists_entity_kind(client, db_session):
    """Verify saved runs persist entity kinds from the frontend state shape."""
    user = User(username="runner", password_hash="hash")
    db_session.add(user)
    db_session.commit()
    db_session.refresh(user)
    ...
    payload = {
        ...
        "state_json": {
            ...
            "player": {"x": 1, "y": 1, "roomId": None, "lightRadius": 0, "torchLit": False},
            "visibility": {"visibleNow": [], "exploredEver": []},
            "lootLog": {"entries": [], "totalValue": 0},
            "generation": {"entranceRoomId": None, "connectivityValid": True},
    ...
    saved_run_id = response_data["id"]

    entities = db_session.exec(
        select(Entity).where(Entity.saved_run_id == saved_run_id).order_by(Entity.id)
    ).all()

    assert [entity.kind for entity in entities] == ["feature", "monster"]
